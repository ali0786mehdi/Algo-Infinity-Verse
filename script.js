...
const practiceProblems = [
  ...
  { id: 15, title: "Course Schedule", difficulty: "medium", tags: ["Graphs", "Topological Sort"], acceptance: "44.7%", category: "graphs", description: "There are numCourses courses (labeled 0 to numCourses-1). Given prerequisites where prerequisites[i] = [a, b] means course b must be taken before course a, determine if you can finish all courses.", constraints: ["1 ≤ numCourses ≤ 2000", "0 ≤ prerequisites.length ≤ 5000", "prerequisites[i].length == 2"], followUp: "Can you return the actual valid course order?", functionName: "canFinish", params: ["numCourses", "prerequisites"], guide: "numCourses: total number of courses (labeled 0 to numCourses-1)\nprerequisites: array of [a, b] pairs where course b must be taken before course a\nreturns: true if all courses can be completed (no cycle in the prerequisite graph), false otherwise\n\nHint: Use Kahn's algorithm (BFS topological sort with in-degree tracking) or DFS with cycle detection", testCases: [{ input: [2, [[1,0]]], expected: true }, { input: [2, [[1,0],[0,1]]], expected: false }, { input: [4, [[1,0],[2,0],[3,1],[3,2]]], expected: true }] },

# Contributing

Thank you for your interest in contributing! Please follow these guidelines to ensure a smooth contribution process.

## Contribution Rules

- **Do not close issues without maintainer approval** - Only maintainers have the authority to close issues.
- **Always link PRs to issues** - Reference the relevant issue number in your pull request description.
- **Follow code style** - Maintain consistency with the existing codebase.
- **Use proper labels** - Apply appropriate labels to your issues and pull requests.

## Pull Request Process

1. Create a branch from `main` with a descriptive name
2. Make your changes following the project's coding standards
3. Ensure all tests pass
4. Update documentation if necessary
5. Submit a pull request with a clear description
6. Wait for review and address any feedback

## Reporting Issues

- Use the appropriate issue template for your report
- Provide clear reproduction steps for bugs
- Include relevant screenshots or logs
- Be respectful and constructive in discussions

## Code of Conduct

This project follows a code of conduct. By participating, you are expected to uphold this code.